...
local function draw(buf, lines)
	...
	if center then
		...
		if widechar then
			intro_dim.x = intro_dim.x / 2
		end
		local diffx = screen_dim.x - intro_dim.x
		local diffy = screen_dim.y - intro_dim.y
		if diffx < 0 then
			vim.notify("switchintro: intro wider than the screen, ignoring centering.", vim.log.levels.WARN)
		elseif diffy < 0 then
			vim.notify("switchintro: intro taller than the screen, ignoring centering.", vim.log.levels.WARN)
		else
